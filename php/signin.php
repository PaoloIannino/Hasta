<?php
	...
	// Suggestion displayed on mouse over "password" input field
	$password_title='Enter a password containing at least one digit and one alphabetic character';
	...
	if($_SERVER['REQUEST_METHOD'] === "POST"){
		if(isset($_POST['email']) && isset($_POST['password'])){
			...
			if(sign_in($email, $password)){ 
				// If the sigin is successuful, update session data and redirect to main view
	...
	echo '	<div id="signin_message_div" class="page_header_div">
				<h1 class="page_header">Sign In
					<div class="access_failure">'	. $error . '</div>
				</h1>
			</div>
			<div id="signin_form_div" class="form_div">
			<form method="post" id="signin_form">
				...
				<h3> Password </h3> 
				<input class="input_box" type="password" id="pwd" name="password" placeholder="Your password"  
						pattern=".*([0-9].*[a-zA-Z])|([a-zA-Z].*[0-9]).*" title="' . $password_title .'" required><br>
				<div id="submit_button">
					<input type="submit" name="submit" value="SIGN IN" class="input_button">
				</div>
				<script type="text/javascript">//<!--
					$("#signin_form").submit(function(event) {
						$("#changeable").load("php/signin.php", $("form").serializeArray(), function(response, status, xhr) {
									if(status == "error")
										ReportError(xhr.status + " - " + xhr.statusText);
						});
				
						event.preventDefault();
						$("#sigin_form").trigger(\'reset\');
					});
				//--></script>
			</form>
		</div>';
